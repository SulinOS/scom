...
import gettext
import subprocess
from scomd_cgroupfs import Cgroupfs
...
class Logger:
    """Logger class for dumping into /var/log/scomd.log."""
    def __init__(self):
        self.lines = ["\n"]
    ...
    def flush(self):
        ...
        try:
            self.lines.append("\n")
            with open("/var/log/scomd.log", "a") as _file:
                _file.writelines(self.lines)
        except IOError:
            UI.error(_("Cannot write scomd.log, read-only file system"))
...
class Config:
    """Configuration class which parses /proc/cmdline to get scomd options."""
    def __init__(self):
        self.fstab = None
        ...
        # Default options for scomd= in /proc/cmdline
        self.options = {
            "language"      : "en",
            "clock"         : "local",
            "clock_adjust"  : "no",
        ...
        if os.path.exists("/etc/conf.d/scomd"):
            self.options.update(load_config("/etc/conf.d/scomd"))
    ...
    def parse_kernel_options(self):
        """Parse scomd= from kernel boot parameters."""
        ...
        options = get_kernel_option("scomd")
...
def set_system_language():
    ...
    # without duplicating default->scomd.conf->kernel-option logic
    # we do here. Note that these are system-wide not per user,
    # and only for reading.
    create_directory("/etc/scomd")
    write_to_file("/etc/scomd/language", "%s\n" % lang)
    write_to_file("/etc/scomd/keymap", "%s\n" % keymap)
    write_to_file("/etc/scomd/locale", "%s\n" % language.locale)
...
def load_translations():
    """Loads the translation catalogue for scomd."""
    global __trans
    global _
    lang = CONFIG.get("language")
...
def get_service_list(bus, _all=False):
    ...
    services = obj.listModelApplications("System.Service",
                                         dbus_interface="tr.org.sulin.scom")
    if _all:
        return services
    else:
        enabled = set(os.listdir("/etc/scomd/services/enabled"))
        conditional = set(os.listdir("/etc/scomd/services/conditional"))
...
def start_udev():
    ...
    ######
    # we don't need that; scomd_tmpfiles and baselayout.conf creates this dir and symlink
...
def mount_remote_filesystems():
    ...
    from sulin.fstabutils import Fstab
    fstab = Fstab()
...
def main():
    ...
    LOGGER.log("/sbin/scomd.py %s" % sys.argv[1])
    ...
    if sys.argv[1] == "sysinit":
        ...
        out = [line for line in capture("/sbin/scomd_tmpfiles.py", "--boot")[0].split("\n") if line.strip()]
...
if __name__ == "__main__":
    if "profile" in get_kernel_option("scomd"):
        import cProfile
        cProfile.run("main()", "/dev/.scomd-%s.log" % sys.argv[1])
    else:
        main()
